...
Node/npm, Python threads, POSIX threads, C, Go, SQLite, and PTYs, but SparkRun
...
2. Runs `sparkrun-toolchain-check --smoke`, records the exact Alpine package
   inventory, and records the Docker builder version.
...
5. Records the disk SHA-256, Dockerfile SHA-256, package-inventory SHA-256,
...
Any filesystem-changing edit to `Dockerfile`, `image.json`, or `bin/` must bump
